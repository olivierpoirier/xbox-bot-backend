...
export default function PlayerBar({
  ...
  /**
   * Animation fluide de la barre de progression via requestAnimationFrame
   */
  useEffect(() => {
    const updateTick = () => {
      const nowMs = Date.now();
      
      // Sécurité : On ne met pas à jour si l'utilisateur manipule le curseur 
      // ou s'il vient de faire un saut (verrou de 800ms pour attendre le serveur)
      if (!isDragging && nowMs - lastSeekTime.current > 800) {
        if (!now) {
          ...
          // CORRECTION : startedAt est le point de départ théorique incluant l'offset.
          // La position actuelle est simplement la différence de temps.
  ...
  // Calcul de la position finale pour l'affichage (priorité au drag)
    ...
    <AnimatePresence>
      {now?.url && (
        <>
          ...
          >
            {/* Barre de progression mini (en haut de la barre) */}
